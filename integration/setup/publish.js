...
const setReg = run('npm config set registry http://verdaccio:4873/');
if (!setReg.ok) process.exit(1);

console.log('[Publish] Setting fake auth token...');
const setAuth = run('npm config set //verdaccio:4873/:_authToken=faketoken');
if (!setAuth.ok) process.exit(1);

console.log('[Publish] Getting registry...');
const getReg = run('npm config get registry');
if (!getReg.ok) process.exit(1);

const versionRes = run('npm pkg get version --json');
...
run(`npm unpublish @texas2010/lib@${version} --force || true`);

console.log('[Publish] Publishing package...');
const pub = run('npm publish --ignore-scripts');
